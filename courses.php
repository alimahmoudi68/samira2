    ...
    <div class="container mx-auto my-3 flex flex-col items-end">
        ...
        <div x-data="products()" @scroll.window="checkScroll()" class='w-full flex flex-col'>
            ...
            <div class='my-scroll w-full flex flex-row gap-x-3 mb-4 pb-2 overflow-x-auto md:overflow-x-visible scroll-smooth'>
                ...
                ?>
                <div x-data="dropdown" class="flex flex-col bg-dark-card-100 rounded-lg relative">
                    <div :class="{'md:border-gray-800' : open}" class='flex items-center gap-x-2 cursor-pointer px-4 py-2 text-text-primary-100 border border-dark-card-100 md:hover:border-gray-800 rounded-md' @click="toggle()">
                        <h4>دسته بندی‌</h4>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" :class="{'rotate-180': open}" class="w-[20px] h-[20px]">
                    ...
                    <div x-show="open" x-cloak x-collapse.duration.100ms  @click.outside="close" class='flex flex-col bg-dark-card-100 md:border md:border-gray-800 fixed left-0 bottom-0 z-[9999999999999999999999999] md:absolute gap-y-3 p-3 rounded-md md:top-[50px] md:bottom-auto md:left-auto right-0 md:z-100000000000000'>
                        <div class='flex items-center gap-x-1'>
                            ...
                            <input x-model="filterElementKey['cat']" class='outline-hidden text-text-primary-100 text-[0.9rem] placeholder:text-[0.9rem] bg-dark-back-100 placeholder:font-light placeholder:text-text-primary-100 p-2' placeholder='جستجو'>
                        </div>
                        <?php
                        ...
                        ?>
                            <div x-show="isShowFilterElement('cat' , '<?php echo $term->name ?>')" class="w-full flex items-center gap-x-2 cursor-pointer" data-id="<?php echo $term->slug ?>" @click="updateFilters('<?php echo $term->slug ?>' , 'cat')">
                                ...
                                <span class='text-text-primary-100'><?php echo $term->name ?>(<?php echo $term->count ?>)</span>
                            </div>
                        <?php 
                        ...
                        <div class='flex items-center justify-center gap-x-2'>
                            <div class='border border-dark-back-100 px-3 py-1 cursor-pointer rounded-md' @click="clearFilter('cat')">
                            ...
                            <div class='px-3 py-1 bg-primary-100 text-text-primary-100 cursor-pointer rounded-md' @click="makeFilterUrl(1)">
                                اعمال
                            </div>
                        </div>
                    </div>
                                ...
                                <div x-data="dropdown" class="flex flex-col bg-dark-card-100 rounded-lg relative">
                                    ...
                                    <div x-show="open" x-cloak x-collapse.duration.100ms  @click.outside="close" class='flex flex-col bg-dark-card-100 md:border md:border-gray-800 fixed left-0 bottom-0 z-[9999999999999999999999999] md:absolute gap-y-3 p-3 rounded-md md:top-[50px] md:bottom-auto md:left-auto right-0 md:z-100000000000000'>
                                        <div class='flex items-center gap-x-1'>
                                            ...
                                            <input x-model="filterElementKey['<?php echo $attributeName; ?>']" class='outline-hidden text-text-primary-100 text-[0.9rem] placeholder:text-[0.9rem] bg-dark-back-100 placeholder:font-light placeholder:text-text-primary-100 p-2' placeholder='جستجو'>
                                        </div>
                                        <?php
                                        $term_data = array(); 
                                        ...
                                        ?>
                                            <div x-show="isShowFilterElement('<?php echo $attributeName; ?>' , '<?php echo $term->name ?>')" class="w-full flex items-center gap-x-2 cursor-pointer" data-id="<?php echo $term->slug ?>" @click="updateFilters('<?php echo $term->slug ?>' , '<?php echo $attributeName ?>')">
                                                ...
                                                <span class='text-text-primary-100'><?php echo $term->name ?>(<?php echo $product_count ?>)</span>
                                            </div>
                                        <?php 
                                        ...
                                        ?>
                                        <div class='flex items-center justify-center gap-x-2'>
                                            <div class='border border-dark-back-100 px-3 py-1 cursor-pointer rounded-md' @click="clearFilter('<?php echo $attributeName ?>')">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" class="w-[24px] h-[24px] stroke-white-100">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                                                </svg>
                                            </div>
                ...
                ?>
                <div x-data="dropdown" class="flex flex-col bg-dark-card-100 rounded-lg relative">
                    <div :class="{'md:border-gray-800' : open}" class='flex items-center gap-x-2 cursor-pointer text-text-primary-100 px-4 py-2 border border-dark-card-100 md:hover:border-gray-800 rounded-md' @click="toggle()">
                        <h4>قیمت</h4>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" :class="{'rotate-180': open}" class="w-[20px] h-[20px]">
                        <path stroke-linecap="round" stroke-linejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" />
                        </svg>
                    ...
                    <div x-show="open" x-cloak class='bg-darkTransparent-100 fixed right-0 left-0 bottom-0 top-0 z-[999999999999999999999999] md:hidden'></div>
                    <div x-show="open" x-cloak x-collapse.duration.100ms  @click.outside="close" class='flex flex-col bg-dark-card-100 text-text-primary-100 md:border md:border-gray-800 fixed left-0 bottom-0 z-[9999999999999999999999999] md:absolute gap-y-3 p-3 rounded-md md:top-[50px] md:bottom-auto md:left-auto right-0 md:z-100000000000000'>
                        <!-- slider price -->
                        <div class='w-full max-w-[300px] mx-auto flex flex-col justify-center items-center px-[16px] pt-[20px]'>
                            <tc-range-slider 
                            value1="0" 
                            ...
                            <div class='flex flex-col mt-[10px]'>
                                <div>
                                    ...
                                    <span x-text='showPriceMin()' class='outline-hidden text-[0.9rem] placeholder:text-[0.9rem] placeholder:font-light placeholder:text-gray-600'></span>
                                </div>
                                <div>
                                    <span>
                                        تا
                                    </span>
                                    <span x-text='showPriceMax()' class='outline-hidden text-[0.9rem] placeholder:text-[0.9rem] placeholder:font-light placeholder:text-gray-600'></span>
                                </div>
                            </div>
                        </div>

                        <div class='flex items-center justify-center gap-x-2'>
                            <div class='border border-dark-back-100 px-3 py-1 cursor-pointer rounded-md' @click="clearPricesFilter()">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" class="w-[24px] h-[24px] stroke-white-100">
                                <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                                </svg>
                            </div>
            ...
            <div class='flex items-center mb-4 md:mb-8 gap-x-3'>
                <template x-for="item in filterToList(filters)">
                    <div class='flex items-center gap-x-3 px-3 py-1 rounded-md bg-gray-500'>
                        <span x-text="getAttribiteName(item.key , item.value)" class='text-text-primary-100'></span>
            ...
            <div class='w-full flex min-[350px]:gap-[10px] flex-wrap relative'>
                ...
                <span x-show="!loading && products.length==0" x-cloak class='mx-auto my-[60px] text-text-primary-100'>
                دوره‌ای پیدا نشد :(
                </span>
 

                <template x-for="product in products">
                    <div class='w-[calc(50%-10px)] md:w-[calc(33%-20px)] lg:w-[calc(25%-30px)] card-product flex flex-col items-center transition-all duration-300 rounded-lg bg-dark-card-100 relative'>
                        ...
                        <div class='w-full flex flex-col p-3'>
                            ...
                            <a x-text='product.title' :href="product.link" class='w-full h-[50px] flex items-start flex-grow text-[1.2rem] font-semibold text-text-primary-100'>
                            </a>
                            <div class='flex items-center'>
                                <div class='flex items-center gap-x-2'>
                                    <div class='flex justify-center items-center rounded-md bg-dark-back-100 text-text-primary-100 p-1'>
                                ...
                                <div class='w-full flex flex-col justify-center items-end text-sm font-light h-[40px]'>
                                    ...
                                    <span x-text="showPrice(product.formatted_sale_price , product.formatted_regular_price)" class='text-text-primary-100 font-medium'></span>
                                </div>
                            </div>
                        </div>
                    </div>
